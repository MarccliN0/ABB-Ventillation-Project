...
void delayMicroseconds(unsigned int us) {
	uint64_t comp;
	comp = (Chip_Clock_GetSystemClockRate() / 1000000) * us; // calculate compare value
	Chip_RIT_Disable(LPC_RITIMER); // disable RIT – compare value may only be changed when RIT is disabled
	Chip_RIT_SetCompareValue(LPC_RITIMER, comp); // set compare value to RIT
	Chip_RIT_SetCounter(LPC_RITIMER, 0); // clear RIT counter (so that counting starts from zero)
	Chip_RIT_Enable(LPC_RITIMER); // enable RIT
	while (!Chip_RIT_GetIntStatus(LPC_RITIMER)) {
	} // wait until RIT Int flag is set
	Chip_RIT_Disable(LPC_RITIMER); // disable RIT
	Chip_RIT_ClearIntStatus(LPC_RITIMER); // clear RIT Int flag
}
